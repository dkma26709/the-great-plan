# Saurus Oldblood


> Ancient — thousands of years old — the apex of the Saurus caste. Not physically *stronger* than a Scar-Veteran; not *faster*; not wielding a better class of weapon. What the Oldblood possesses is **age**. The latent strategic knowledge of war, implanted at spawning by the Old Ones, unlocks steadily across a Saurus's lifetime. The Oldblood carries millennia of unlocked understanding. He cannot articulate a manoeuvre in words — but when he moves, the entire Lizardmen line reorients around his decisions. Attended by scores of Skinks who adorn him with bronze armour-plates, warpaint, oils, and the shrunken heads of sacrificed foes.

**Saurus Oldblood:**

| M | WS-A | WS-D | BS | S | T | W | I | Res | LiS | US |
|---|----|----|----|---|---|---|---|----|-----|----|
| 4 | 7 | 7 | 0 | 5 | 5 | **8** | 3 | 10 | 2 | 4 |

**Points:** **285** base (on foot, with Celestite Blade + shield, no Light Armour)

- **Keywords:** Lizardmen, Saurian, Character, Lord
- **Unit type:** Infantry (Character); may take a mount
- **Natural Armour:** **3+** (thickened Oldblood scales — denser than rank-and-file Saurus, but not the layered bronze-plate fortress that NA 2+ would imply. The Light Armour upgrade brings the Oldblood up to Bastiladon-grade saving)
- **Ward Save:** — (none baseline; magic items can grant)
- **Equipment (default):** Celestite Blade + shield
- **Equipment options** (pick one melee):
  - Additional Celestite Blade (+1 A to Celestite Blade profile)
  - Celestite Spear (2" reach, AP -2)
  - Celestite Polearm (2" reach, Two-Handed, S+2, AP -3)
  - Celestite Great Weapon (1" reach, Two-Handed, S+3, AP -4, D 2) — no shield
  - **Light Armour** (+15 pts) — Skink-attendant-applied bronze armour-plates worked into the scales. Combined save: NA 3+ + Light Armour +1 step = **2+** (Bastiladon-grade). Lore-direct: "scores of Skinks secure bronze armour-plates" — the upgrade makes the canonical Skink-attended battle-prep mechanically explicit.
  - Magic items up to 100 pts
  - Blessed Spawnings — up to 3, per-character costs from §10 menu (Mark of the Old Ones counts as one slot)
- **Mount options:**
  - **Horned One** (see Horned One Riders mount profile — single-model)
  - **Aggradon** (see Aggradon Lancers mount profile — single-model)
  - **Carnosaur** (see Character Mounts below)
- **Battle Standard option:** +25 pts — one Saurus Oldblood in the army may carry the Army Battle Standard. Banner may be a magic banner with no points limit; the Oldblood's other magic item allowance remains intact (taking a magic banner does **not** reduce the Oldblood's magic-item budget)
- **Command options:** none (single-model character)
- **Special rules:**
  - Disciplined (see §8)
  - Cold-Blooded (see §8)
  - **Stubborn** (see §8) — Saurus species
  - **Predatory Fighter** (see §8) — Saurus natural attacks trigger extra attacks on unmodified 6s to hit
  - **Fear** (see §8)
  - **Locked Jaws** *(Saurus species rule)* — when this Oldblood is reduced to 0 W during the Combat phase, before being removed it makes 1 final Saurus Jaws attack against the model that scored the killing blow (or closest enemy in base contact). Single attack — no Predatory Fighter cascade on the death-bite. The lore-canonical "even in death the bite still goes in," propagated from the Saurus species pass.
  - **Primeval Command** *(identity rule)* — friendly **Saurus** units with any model within **18"** of the Oldblood gain **+1 Res** for stress threshold purposes. Additionally, at the start of any friendly turn, the Oldblood nominates one friendly Saurus unit within 18"; that unit immediately removes **2 stress counters**. Represents the latent strategic knowledge of war unlocked across the Oldblood's centuries — his people orient around his unspoken tactical vision.

**Weapon profiles (A on weapons):**

- **Saurus Jaws** *(Oldblood natural, always)* — 1" | **A 2** | S 5 | AP -2 | D D3 — ancestral bite; strength unchanged from Scar-Veteran tier (age does not grow the jaws) `[Natural]`
- **Tail Sweep** *(natural, rear arc only — disabled when mounted on a Carnosaur, Aggradon, or Horned One per cavalry geometry)* — **1" reach** | **A 1** | S 5 | AP -1 | D 1 — heavy crocodilian tail. Punishes rear-arc flanking on a foot Oldblood. Resolved at I 3. `[Natural]`
- **Celestite Blade** *(default, 1H)* — 1" | **A 4** | S+1 (= S 6) | AP -2 | **D 2** — Oldblood tier upgrades weapon **Damage** by +1 over Scar-Veteran (Scar-Vet had D 1 here). Represents precision and placement — each blow dispatches a foe — not greater strength `[1H Blade]`
- **Celestite Spear** *(option, 1H)* — 2" | A 4 | S+1 | AP -2 | D 2 `[1H Reach]`
- **Celestite Polearm** *(option, 2H)* — 2" | A 4 | S+2 | AP -3 | D 2 `[1H Reach]`
- **Celestite Great Weapon** *(option, 2H)* — 1" | A 3 | S+3 | AP -4 | D 2 `[2H]`

> **Lore notes:** Thousands of years old — the apex of the Saurus caste. Crucially, *not* physically stronger than a Scar-Veteran; what the Oldblood possesses is **age**. The Old Ones implanted latent strategic knowledge of war at spawning that unlocks across a Saurus's lifetime; the Oldblood carries millennia of unlocked understanding, cannot articulate a manoeuvre in words, but when he moves, the entire line reorients around his decisions. Differentiation from Scar-Veteran sits on WS 7 (the absolute top melee skill in the roster), W 8 (centuries of survival), NA 3+ thickened scales, and weapon Damage +1 — *precision* of placement, not greater strength (S 5 / T 5 / I 3 unchanged). **Primeval Command** is the identity rule: 18" Saurus Res aura + per-turn stress-strip on a nominated unit, the latent strategic knowledge enacted as morale anchor. The **Light Armour** upgrade is the lore-direct "scores of Skinks secure bronze armour-plates" — Skink-attended battle-prep made mechanically explicit. Inherits Locked Jaws and Tail Sweep from the species pass.
