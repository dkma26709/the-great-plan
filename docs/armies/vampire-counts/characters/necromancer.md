# Necromancer


> A mortal mage devoted to the Lore of Necromancy — an apprentice or independent practitioner of dark magic, ranging from young scholars who have only just opened the forbidden tomes to old loremasters tied to a Vampire Court. Necromancers are *human*, not Undead — they age, they tire, they take normal stress and can be Broken. But their **role is keeping the Undead on the table** via Necromancy spells, particularly Invocation of Nehek. A Vampire Counts army without a caster (Vampire or Necromancer) bleeds to nothing within a few turns; the Necromancer is the cheaper alternative to a Vampire Hero when the budget cannot afford one.

**Necromancer:**

| M | WS-A | WS-D | BS | S | T | W | I | Res | LiS | US |
|---|----|----|----|---|---|---|---|----|-----|----|
| 4 | 3 | 3 | 3 | 3 | 3 | 3 | 3 | 8 | 1 | 1 |

**Magic Profile (level-dependent):**

| Level | Lore Access | Cast Base | Dispel Base | Channelling | Cast Bonus | Dispel Bonus | Dispel Range | SK |
|-------|----|-----|-----|----|----|----|-----|-----|
| **Lvl 1** *(base)* | 1 | 2 | 1 | 1 | +0 | +0 | 18" | **2** |
| **Lvl 2** *(+30 pts)* | 2 | 2 | 1 | 2 | +1 | +0 | 18" | **2** |

*Lvl 1 caster (Lore Access 1). Upgrade to Lvl 2 at list-build for +30 pts. **Lores Known: Necromancy** only (single-lore, no choice — the Necromancer is defined by the dark school he studies). **SK 2** = total spells known: Signature spell (free, from Necromancy) + 1 additional from Necromancy.*

**Points:** **75** base (Lvl 1, hand weapon, no armour). **+30** to Lvl 2 (= 105 pts).

- **Keywords:** Vampire Counts, Human, Wizard, Character, Hero
- **Unit type:** Infantry (Character)
- **Natural Armour:** —
- **Worn armour:** — (Necromancers typically eschew armour; the dark robes are a focus item, not protection)
- **Combined save:** —
- **Equipment (default):** Hand weapon (ritual dagger / staff)
- **Equipment options:**
  - Magic items up to **50 pts** (Hero-tier)
- **Mount options:** Skeletal Steed, Corpse Cart *(pending Character Mounts / Special-unit write-up)*
- **Battle Standard option:** — *(wizards do not take BSB by convention)*
- **Special rules:**
  - Disciplined (see §8)
  - **Wizard (Lore Access 1 / 2 by level)** — see Magic Profile. Lore: Necromancy intrinsic
  - **No Undead keyword** — Necromancer is a *living human* mage; takes normal stress, can be Broken, can Rally, takes normal Fear / Terror tests. This is mechanically meaningful — a Necromancer alone in front of an enemy unit *can* break and flee; the Vampire's Undead chassis does not
  - **No Vampiric Hunger** — Necromancer does not feed on the slain (he is mortal)
  - **Master of Death** *(identity rule)* — when this Necromancer casts **Invocation of Nehek** (the Lore of Necromancy Signature spell, see §11), the **spell's Casting Value is reduced by 1** (CV 5 → CV 4 when this Necromancer casts it). Represents the Necromancer's specialised devotion to the resurrection lifeline that defines his role on the battlefield — he doesn't have the Vampire's three-lore breadth, but his focused command of Invocation makes the keystone resurrection spell more reliable for him than for any other caster in the army. CV reductions stack: a Necromancer casting on Skeletons sees Invocation's CV reduced by both Master of Death (-1) and Bone Tide (-1) for an effective CV 3 — very reliable; on Grave Guard the reductions partially offset Lingering Souls (Master of Death -1, Lingering Souls +1, net CV 5 unchanged from baseline)
  - **Voice from the Grave** *(identity rule)* — once per battle, in any friendly Magic phase, the Necromancer may consume **D3 wounds from one friendly Skeleton Warriors / Zombies / Grave Guard unit within 12"** (the unit takes the wounds; the Necromancer chooses which unit) to gain **+1 to a single casting roll** that phase. The forbidden bargain literalised — the Necromancer fuels the spell with his own dead, scouring bones and rot to feed the casting. The wounds are taken before the casting roll resolves; if the chosen unit is destroyed by the cost, that's a bargain made. May be used once per battle regardless of level.

**Weapon profiles (A on weapons):**

- **Hand weapon (ritual dagger or staff)** *(default, 1H)* — 1" reach | **A 1** | S 3 | AP 0 | D 1 `[1H Blade]`

> **Lore notes:** A mortal mage devoted to the Lore of Necromancy — apprentice or independent practitioner of dark magic. **No Undead keyword** is the critical mechanical caveat: Necromancer breaks and flees like a normal human, takes Fear / Terror, can be Rallied. He's the *master* of Undead, not one of them. The Vampire Counts equivalent of the Empire Battle Wizard — single-lore Hero caster with the same level-up ladder; structural difference is *lore-pick pattern* (Battle Wizard picks one of eight Winds, Necromancer is locked to Necromancy intrinsic). **Master of Death** is the identity rule: when this Necromancer casts Invocation of Nehek, its CV is reduced by 1 — focused devotion to the resurrection lifeline that defines his role; stacks with Bone Tide on Skeletons (effective CV 3, very reliable). **Voice from the Grave** is the once-per-battle forbidden bargain: consume D3 wounds from a friendly Undead unit within 12" to gain +1 to a casting roll — the Necromancer fuels the spell with his own dead. Without a Necromancer or Vampire on the table, a Vampire Counts army bleeds off via Crumble within 2-3 turns; the Necromancer is the cheap-budget caster slot for keeping Undead alive.
