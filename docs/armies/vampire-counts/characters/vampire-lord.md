...
- **Hand weapon** *(default, 1H with shield)* — 1" reach | **A 4** | S 5 | AP -1 | D 1 — vampire-grade swordwork; AP -1 reflects centuries of practice and supernatural strength `[1H Blade]`
- **Great weapon** *(option, 2H)* — 1" reach | A 4 | S+2 (= S 7) | AP -2 | D 1 | Two-Handed `[2H]`
- **Lance** *(option, mounted only)* — 2" reach | A 4 | charge: S+2, AP -2, D 2; subsequent: S+0, AP 0, D 1 `[1H Lance]`
- **Bite** *(natural, always available — once per melee round)* — 1" reach | **A 1** | S 5 | AP -1 | D 1 — vampire's iconic fanged strike; resolved at the Vampire's I (no I bonus or penalty); does *not* trigger Red Fury or other "Vampire's weapon" effects — it's a per-round bonus attack tied to the Vampire's nature, not a standard weapon. **Feeding trigger:** an unsaved wound caused by this Bite triggers **Vampiric Hunger** (regen 1 W, capped 1 W/turn — see Faction Mechanics). The Bite is the Vampire's only feeding mechanism; sword-strikes draw blood, only the fang drinks `[Natural]`

> **Lore notes:** Stats per heft calibration — WS 7 / BS 5 / S 5 / T 5 / I 7 / A 4 reference unchanged. **W 3 → 5** (Lord-tier wound bump; vampire supernatural toughness; below Saurus Oldblood W 7 since vampires are humanoid scale, not Saurus apex bulk). **Res 10 reference unchanged** — already at the cap. Heavy armour + shield default 3+ save; great weapon swap drops to 4+. **Magic Profile** is the level ladder — Lvl 2 baseline (Lore Access 2) at Cast Base 2 / Dispel Base 1 / Channelling 2 / Cast Bonus +1 / Dispel Bonus +0 / Dispel Range 18", upgradable to Lvl 3 (Lore Access 3) or Lvl 4 (Lore Access 4) for +35 pts each. Lvl 4 Vampire Lord matches Wizard Lord Lvl 4 magic stats nearly exactly — both reach Lore Access 4 Master tier; Vampire Lord's edge is **three intrinsic lores** (Death + Necromancy + Shadow) vs Wizard Lord's single-Wind. With Forbidden Lore the Vampire Lord exceeds even Slann breadth on generic Winds (8 generics + Necromancy), at lower Cast Base / Dispel Base / Channelling / Dispel Range than Slann. **Vampiric Hunger** intrinsic + **Bloodline Powers** menu = the vampire's identity expression: the universal trait + customised power layer. Compares to Saurus Oldblood (260 pts, NA 2+, Predatory Fighter, Primeval Command 18" Res aura, no magic): Vampire Lord 280 pts, no NA but 3+ worn save + magic items + Bloodline Powers, Lvl 2-4 Wizard with multi-lore — different mechanical fingerprint, both Lord-tier slot. **Bite weapon profile** is the iconic vampire flair — once per melee round, S 5 AP -1 D 1, tied to the model's nature rather than equipment (so it applies regardless of weapon swaps). Doesn't trigger Red Fury (avoids cascading-loop concerns).
