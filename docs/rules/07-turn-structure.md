...
- **In subsequent turns:** the opponent may use one of their Magic phase activations to attempt to dispel the RIP spell using the standard *Dispelling* procedure above (lead dispeller within DR, support contributions, invest dice from personal/shared pools, bonus D6s on natural 6s, total compared to the spell's original cast result). Only one dispel attempt per RIP spell per turn.
