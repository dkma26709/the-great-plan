# Lore of Cosmic Architecture


> The celestial counterpart to Geomancy. Where Geomancy reaches into the bedrock, Cosmic Architecture reaches into the **cosmos** — reading the Old Ones' blueprint inscribed in the stars, bending reality to match the ancient plan. The lore's five pillars: **foresight** (re-rolls, prescience, fate-loaning), **celestial strikes** (comets, starfall), **reality conveyance** (Walk Between Realms, mid-battle teleportation), **fate-binding** (constraining enemies to the cosmic pattern), and **cosmic equilibrium** (anti-magic, balance-of-reality). Flavour register: silver, gold, the colour of distant stars, the hum of vast cosmic patterns visible only to minds vast enough. Where Heavens reaches for the *sky*, Cosmic Architecture reaches for the *design behind the sky*.

**Lore Attribute — Prescience:** whenever this wizard **successfully casts a Cosmic Architecture spell**, the caster designates one friendly unit within **18"**. That unit gains a **Foresight token** — spendable at any time before the caster's next Magic phase to **re-roll any single failed roll** (to-hit, to-wound, save, charge distance, stress threshold). One token per unit (new tokens overwrite old). The Slann loans a glimpse of the future to their army, one moment at a time.

**Signature Spell — Cosmic Foresight**

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 6 | 18" | Augment | Until start of next turn |

Target friendly unit **ignores enemy cover bonuses** on all its attacks (melee and ranged) until start of next turn. The Slann shares prescient vision; intervening cover, trees, walls, and half-concealment are irrelevant — the target unit's attacks find flesh regardless of what hides it.
...
D3 caps distribution (1-3 tokens) rather than blanketing every unit in range. A lone Slann gets modest value; one surrounded by multiple combat-critical units gets more reach.
...
The Slann reaches into the enemy's mind and sets a cosmic seal across a specific memory. Strategic — denies the enemy their best spell for a full turn. Works against any spell the target knows (signature or tiered).
...
Reposition rather than redeployment — the unit slips through a cosmic path and re-emerges nearby. Useful to exit combat, slide into a flank, dodge an incoming charge, or pop onto a hill for firing position. Deliberately weaker than Geomancy's Telepathic Summons (which places the unit near the caster, not near the original position); distinct from Life's Paranoth's Pathway (which is delayed but unbounded).
...
Distinct from Comet's Call: Comet Barrage is **instant** and **multi-target** (diffused damage across 2-4 units); Comet's Call is **delayed single-point AoE** (concentrated damage with arrival uncertainty). Caster picks which tool fits the situation — finish multiple wounded units now, or threaten massive damage later.

**Stellar Tempestos Dome**
