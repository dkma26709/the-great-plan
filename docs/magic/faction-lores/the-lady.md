# Lore of the Lady


> The magic of the Lady of the Lake — the goddess of nobility, faith, and the chivalric order. Her servants (Damsels of the Lady, Prophetesses of the Lady) channel a fae-touched magic that *protects, heals, blesses,* and — when the unholy press too close — *purges*. Lady magic is the only general-style lore in the ruleset whose primary mode is *defensive and restorative*; offensive options exist but are scarce, and tilted exclusively against Daemons, Undead, and Chaos. The Lady protects the faithful; she does not raze cities. Available **only** to characters with the **Lady's Chosen** keyword (Damsels of the Lady, Prophetesses of the Lady — see §17). Knight-tier casters do not exist in Bretonnia; the chivalric order is the *recipient* of the Lady's magic, not its channel.

**Lore Attribute — Lady's Touch:** when this wizard successfully casts a Lore of the Lady spell, the caster picks **one friendly Bretonnia unit within 12"** of her:
- **If the chosen unit does NOT have Blessing of the Lady:** the unit gains the **Blessing of the Lady** keyword until the start of the wizard's next turn. The Blessing keyword is granted as an *identity tag* — it does **not** confer the standard Ward Save (6+/5+) from this lore attribute alone. The keyword grant is useful primarily for rule interactions: other Lady spells (e.g., Aegis of the Lady) and any future rules that key off the Blessing keyword treat the unit as Blessed for the duration. Effectively a "preparation" effect, not a defensive boost on its own
- **If the chosen unit already has Blessing of the Lady** (any Vow tier, including Grail Vow's improved Blessing): the Damsel removes **1 stress** from the unit. No test required. The Lady's voice steadies the faithful — a moment of divine calm in the middle of the battle's noise

Single-target, low-power per-cast effect — neither extends mechanical Blessing protection to peasants nor improves existing Blessing. Captures "the Damsel's small voice in the army's ear" without making the lore attribute a power-multiplier.

**Signature Spell — Chalice of Blessings**

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 6 | 18" | Augment | Until start of next turn |

Target friendly Bretonnia unit gains **Ward Save (6+)** until start of next turn AND removes **1 stress** from the unit on cast. The Damsel raises the chalice of holy water blessed by the Lake — divine intervention adds a layer of protection across the chosen unit, and the Lady's voice steadies their fear. The 6+ Ward stacks per §8 Ward Save Stacking with an existing Blessing for a one-step improvement (Knight's Vow 5+ vs S 5+ → 4+) without overshooting; the spell remains modest as a stand-alone Ward and useful as a stacking layer, while the stress removal is the consistent benefit for any target — even peasants gain a moment of holy calm.

#### Basic (Tier 1) — Lore Access 1+

**Lifebloom of the Lady**

| CV | Range | Type |
|----|-------|------|
| 7 | 18" | Augment, Instant |

Target friendly Bretonnia unit restores **D3 wounds**. Apply to wounded models first; excess resurrects slain models in base contact with the unit. **Blessing tier:** if the target unit has the Blessing of the Lady, restores **D3+1 wounds** instead. The Lady's blessing flows through the caster's hands — torn flesh knits, broken bone mends, the faithful return to her service. The truly Blessed are restored more readily.

**Shield of Combat**

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 8 | 18" | Augment | Until start of next turn |

Until start of next turn, attacks against the target friendly Bretonnia unit have their **AP reduced by 1** (an attack at AP -2 resolves at AP -1; AP -1 at AP 0; AP 0 stays AP 0 — no minimum-positive AP, the spell only ever lessens penetration, never adds it). **Blessing tier:** if the target has the Blessing of the Lady, the AP reduction is **2** instead (AP -3 → AP -1; AP -2 → AP 0; etc.). The Lady's hand turns the edge of the blade aside — the deepest blows skim across armour where they would otherwise bite. Pairs powerfully with the Bretonnian heavy-armour-and-shield baseline (2+ saves on Knight cavalry); against the right opponent the spell turns the chivalric line into a near-impenetrable wall.

**Mantle of the Lake**

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 8 | 18" | Augment | Until start of next turn |

Target friendly Bretonnia unit gains **-1 to-Hit against incoming ranged attacks** until start of next turn. **Blessing tier:** if the target has the Blessing of the Lady, ranged attackers also suffer **-1 to-Wound** against the unit (in addition to the -1 to-Hit). A shimmering veil of lake-mist drifts around the unit — arrows lose their line, bolts lose their certainty, gunpowder shots scatter wide. The Lady's faithful are doubly veiled — incoming arrows deflect off the mist *and* find their mark less reliably even when they connect. Defensive shimmer against any ranged source, Magical Attacks included.

**Bladewater Sanctification** *(Blessing-only)*

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 9 | 12" | Augment | Until start of next turn |

**Restriction:** target unit must have the **Blessing of the Lady** keyword. Cannot be cast on peasant units (Men-at-Arms, Longbowmen, Bidowers, Trebuchet Crew) unless they have been previously granted the Blessing keyword (e.g., via the Lady's Touch lore attribute earlier this turn, or via Aegis of the Lady's aura).

Target friendly Blessed Bretonnia unit's melee weapons gain the following until start of next turn:
- **Magical Attacks** (see §8) — useful against Ethereal targets and any rule requiring a magical attack source
- **Always wound on at worst 5+** — the unit's melee to-Wound rolls require **at most 5+** regardless of Strength vs Toughness. Even a S 3 unit attacking T 7 wounds on 5+ (rather than the standard 6+ floor or unable-to-wound result). Captures "the Lady's water sanctifies the blade — every blow registers against any target, however hardened"

The Damsel sprinkles consecrated lake-water on the chosen unit's blades and lance-heads, channelling the Lady's recognition of her sworn warriors. The sanctification is reserved to those already Blessed — peasants cannot bear the Lady's water without a Vow first opening them to it. Pairs naturally with Questing Knights' permanent Anointed Steel for a one-round wound-floor boost on top of their always-Magical great weapons.

#### Intermediate (Tier 2) — Lore Access 2+

**Silver Mirror**

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 9 | Self | Augment | Until start of next turn |

The caster (and any unit she is attached to) gains **Ward vs Spells (4+)** until start of next turn. The Damsel raises a silver mirror inscribed with Lady-runes — hostile magic is reflected back into the caster's pocket of reality, sometimes finding its caster instead of its target. Defensive against magical attacks across the phase.

**Flickering Fires of the Faerie**

| CV | Range | Type |
|----|-------|------|
| 9 | 24" | Magic Missile, Instant |

Target enemy unit suffers **D6 hits at S 4, AP -1, D 1, Magical Attacks**. Faerie-fire dances around the target — ethereal blue-white flame, briefly real, biting at exposed flesh and faltering steel. The lore's only general-purpose damage spell. The Lady is healing and protection; offensive magic is scarce in her repertoire.

**Lake's Embrace**

| CV | Range | Type |
|----|-------|------|
| 11 | 24" | Augment, Instant |

Target friendly Bretonnia unit restores **D6 wounds** AND removes **D3 stress**. **Blessing tier:** if the target has the Blessing of the Lady, restores **D6+2 wounds** AND removes **D3 stress** instead. The unit briefly experiences submersion in the Lady's lake — a moment of holy water, restoration of wounds, dissolution of fear. The truly Blessed are healed more deeply; their submersion finds more of the Lady's restoration. Higher CV than Lifebloom; the Lake's Embrace is the larger blessing, healing more comprehensively while also restoring morale.

#### Advanced (Tier 3) — Lore Access 3+

**Aegis of the Lady** *(RIP, aura)*

| CV | Range | Type |
|----|-------|------|
| 13 | Self | Augment (aura), Remains In Play |

*Remains In Play.* While in play, all friendly Bretonnia models within **12"** of the caster have their **Blessing of the Lady improved by one step** for as long as the spell remains active:
- Knight's Vow Blessing: 6+/5+ → **5+/4+**
- Crusader's Vow Blessing: 6+/5+ → **5+/4+**
- Questing Vow Blessing: 6+/5+ → **5+/4+**
- Grail Vow Blessing (improved baseline 5+/4+): → **4+/3+**

Bretonnia models *without* a Vow (peasants — Men-at-Arms, Longbowmen, Bidowers, Trebuchet Crew) within 12" gain a **Blessing of the Lady at 6+/5+** (the standard Knight's Vow Blessing) for the duration. The lore's signature aura — the caster channels the Lady's protection across the chivalric line, lifting the entire army's faith-defence by one tier. Major positional consideration: keep the caster within 12" of the main line.

**The Lady's Champion** *(Blessing-only)*

| CV | Range | Type | Duration |
|----|-------|------|----------|
| 12 | 18" | Augment | Until start of next turn |

**Restriction:** target must be a friendly Bretonnia **Character** with the **Blessing of the Lady** keyword (effectively all Bretonnia Lord / Hero characters via their Vow or Lady's Chosen status; the Damsel cannot bless a foreign general).

Until start of next turn, the chosen Character may **reroll any natural roll of 1** on:
- **To-Hit** rolls in melee
- **To-Wound** rolls in melee
- **Armour save** rolls
- **Ward save** rolls (including Blessing of the Lady)

The Lady briefly steadies the chosen knight — every die that would have failed at its weakest result is cast again under her gaze. Single-character augment, four-axis benefit. Pairs cleanly with the Paladin's Champion of the Realm identity rule (which already rerolls failed to-Hit vs Characters); together the Paladin gets blanket reroll-to-Hit vs Characters AND reroll-1s on every other die. Captures "the goddess's brief intervention in the chosen knight's combat" without spike-buffing his attack output beyond consistency.

**Holy Cleansing**

| CV | Range | Type |
|----|-------|------|
| 12 | 24" | Magic Missile, Instant |

Target enemy unit suffers **D6 hits**. Against units with the **Daemon**, **Undead**, or **Chaos** keyword, hits resolve at **S 5, AP -2, D 1, Magical Attacks, Flaming Attacks**. Against any other unit, hits resolve at **S 3, AP 0, D 1, Magical Attacks** only — the spell is *purgative*, calibrated to harm the unholy specifically; mortal flesh is barely scorched. The Lady's wrath against the corrupt; the lore's single anti-evil offensive option.

#### Master (Tier 4) — Lore Access 4+

**The Lady's Embrace** *(Blessing-only)*

| CV | Range | Type |
|----|-------|------|
| 18 | 24" | Augment, Instant |

**Restriction:** target unit must have the **Blessing of the Lady** keyword. The Lady's most complete gift extends only to her sworn faithful — peasant units (without Blessing) cannot benefit from this Master-tier restoration unless first granted the keyword via the Lady's Touch lore attribute, Aegis of the Lady, or another rule that confers Blessing.

Target friendly Blessed Bretonnia unit is **fully restored** — every wounded model returns to its starting wound count, every slain model returns to the unit (positioned in coherency with surviving models, or in base contact with the formation). The unit must have started the battle at full strength; The Lady's Embrace can never restore *more* than was originally lost. Command Group models (Leader / Standard Bearer / Musician / Grail Monk if attached) return if they fell. The Lady's most generous gift — a unit emerges from her embrace as if the morning's first muster had only just begun. CV 12 makes the spell hard to land reliably even for a Lord-tier wizard; the most powerful healing in the ruleset, befitting Master tier and the chivalric faithful alone.

**Wrath of the Faerie** *(area effect)*

| CV | Range | Type |
|----|-------|------|
| 18 | 18" (self-aura) | Direct Damage, Instant |

All enemy units with at least one model within **18"** of the caster suffer **D3 hits** at the standard profile (S 5, AP -1, D 1, Magical Attacks). Units with the **Daemon, Undead,** or **Chaos** keyword suffer **D6 hits** instead, at **S 6, AP -2, D 1, Magical Attacks, Flaming Attacks**. The Lady's faerie-host descends in a furious wave — the unholy take the worst of it, but no enemy near the caster is untouched. CV 14 is among the highest in the ruleset; landing this reliably requires Lvl 4 mastery and ideally a magic-item or aura buff. The lore's offensive capstone — paired with The Lady's Embrace's defensive capstone, the Master tier offers either *complete unit restoration* or *cleansing-damage burst*.

> **Design note on Lore of the Lady.** Bretonnia's faction lore, restricted to Damsels and Prophetesses (Lady's Chosen keyword). The lore deliberately mirrors **Lore of Life** in tone — healing-and-protection focused, scarce offensive options — with two key faction-distinct twists: (1) **Direct synergy with Blessing of the Lady** (multiple spells stack with or improve the chivalric Ward), and (2) **Anti-Daemon / Undead / Chaos offensive bias** (the Lady's wrath specifically targets the corrupt; mortal-vs-mortal damage is intentionally weak). The Lord-tier Prophetess unlocks Master-tier access (Lady's Embrace and Wrath of the Faerie), making her the apex caster of the chivalric tradition — comparable to Wizard Lord or Skink High Priest in raw casting power, but with a lore that turns the chivalric army into a *self-restoring, divinely-shielded* line. Pairs naturally with Bretonnia's high-W cavalry — KotR / Knights Errant / Pegasus Knights all have W 2-3 combined per model, and Lady healing keeps them on the field through extended combat. Cross-faction comparison: Skink High Priest and Wizard Lord can heal incidentally via Life access, but the Bretonnian Prophetess gets *all 12 Lady spells* plus the Lord-tier Master content, making her the definitive faction-bound healer-caster. Damsel of the Lady (Hero-tier) gets Lore Access 1-2 access, capping at Intermediate-tier spells — sufficient for army-screening healing and basic Ward buffing without reaching the apex defensive auras (Aegis) or unit-restore (Lady's Embrace).
>
> **Structural note: no Knight-tier casters in Bretonnia.** Per WAP and our §17 design, only Lady's Chosen characters (Damsel, Prophetess) cast Lady magic. Knight-tier characters (Lord, Paladin, KotR, Knights Errant, Questing Knights, Pegasus Knights) are recipients of Lady magic, not casters. This is structurally distinct from Empire (where any noble can train at the Colleges of Magic) or Lizardmen (where any Skink can become a Priest of an Old One); Bretonnia's chivalric order is specifically excluded from magical channelling — the knight's role is *to be blessed*, not to *bless*. Mechanically this concentrates Lady magic in one or two characters per army; tactically this means Lady casters are high-priority enemy targets and high-positional-priority for the Bretonnian player.
>
> **Design note on CV / Range placeholders.** Spell CVs and ranges in this entry are first-pass placeholders. The broader §11 magic system is pending an audit pass to update CV thresholds, range diversity, and difficulty calibration across all lores. When that audit lands, expect Lady spell CVs and ranges to shift — possibly significantly. The current values are designed to feel proportional within the lore (Signature/Basic moderate CV, Master high CV) but are not anchored to a finalised magic-system numerical spec. If a Lady spell here lines up suspiciously with a Heavens or Life or Light spell value, that's drift from the legacy spell drafts rather than a deliberate cross-lore parity statement.

---
